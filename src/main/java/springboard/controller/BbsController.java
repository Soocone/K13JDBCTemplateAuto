...
public class BbsController {
	...
	@RequestMapping("/board/writeT.do")
	...
	public String editAction(Model model, HttpServletRequest req, 
		...
		/*
		 request 내장객체와 수정페이지에서 전송한 모든 폼값을 저장한
		 DTO객체를 Mode에 저장한 후 서비스 객체로 전달한다.
		 */
		model.addAttribute("req", req);
		model.addAttribute("springBbsDTO", springBbsDTO);
		...
		/*
		 ...
		 파라미터가 필요하다 Model 객체에 저장한 후 redirect하면 자동으로
		 쿼리스트링 형태로 만들어 준다.
		 */
		model.addAttribute("idx", req.getParameter("idx"));
		model.addAttribute("nowPage", req.getParameter("nowPage"));
